...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        x = self.place
        stp=0
        while stp<self.min_range and x!=None:
            stp+=1
            x=x.entrance

        while x!=None and stp<=self.max_range:
            if x.is_hive:
                return None
            if len(x.bees):
                # print("DEBUG",x.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        # print("DEBUG",amount,self.health)
        p=amount
        ...
        if self.chew_countdown>0:
            ...
            if x!=None:
                ...
                for q in lst:
                    if q!=x:
                        tmp.append(q)
                Insect.reduce_health(x,x.health)
                self.place.bees = tmp
# END Problem 7
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not QueenAnt.has_instance:
            QueenAnt.has_instance = True
    ...
    def action(self, gamestate):
        ...
        while nowplace!=None:
            x = nowplace.ant
            if x!=None and not isinstance(x,QueenAnt):
                x.buff()
                if isinstance(x,ContainerAnt) and x.ant_contained!=None:
                    x.ant_contained.buff()
